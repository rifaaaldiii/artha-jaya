<x-filament-panels::page wire:poll.1s="refresh">
    ...
    @else
        @php
            $user = auth()->user();
            $rawRole = $user?->role;
            $normalizedRole = $rawRole ? str_replace(' ', '_', strtolower($rawRole)) : null;
            ...
            if (in_array($normalizedRole, ['administrator', 'admin_toko'], true)) {
                $selectedStatusKeys = array_keys($allStatuses);
            } elseif ($normalizedRole && array_key_exists($normalizedRole, $roleStatusMap)) {
                $selectedStatusKeys = $roleStatusMap[$normalizedRole];
            } else {
                $selectedStatusKeys = array_keys($allStatuses);
            }
            ...
            $statuses = [];
            $stepCounter = 1;
            foreach ($selectedStatusKeys as $statusKey) {
                if (! isset($allStatuses[$statusKey])) {
                    continue;
                }

                $statuses[$statusKey] = $allStatuses[$statusKey];
                $statuses[$statusKey]['step'] = $stepCounter++;
            }
            ...
            if (! array_key_exists($currentStatus, $statuses)) {
                $currentStatus = array_key_first($statuses);
            }
                ...
                @foreach($statuses as $statusKey => $statusInfo)
                    ...
                    <div class="prog-step-row">
                        ...
                        <div class="prog-step-circles-col">
                            <div>
                                @if($selesaiStepIsCheck && $loop->last)
                        ...
                        <div style="flex:1;padding-top:4px;">
                            <div class="prog-step-label
                                @if($isCurrent) current @elseif($isCompleted || ($selesaiStepIsCheck && $loop->last)) completed @else upcoming @endif">                                                                 
                                {{ $statusInfo['label'] }}
                            </div>
                ...
                <div class="detail-header">
                    <div style="flex:2;">
                        ...
                        <div class="detail-header-status">
                            @php
                                $badgeMap = [
                                    'produksi baru' => 'status-badge status-red',
                                    'siap produksi' => 'status-badge status-blue',
                                    'dalam pengerjaan' => 'status-badge status-yellow',
                                    'selesai dikerjakan' => 'status-badge status-green',
                                    'lolos qc' => 'status-badge status-indigo',
                                    'produksi siap diambil' => 'status-badge status-purple',
                                    'selesai' => 'status-badge status-bg-default',
                                ];

                                if($normalizedRole === 'admin_gudang') {
                                    $badgeClass =
                                        $currentStatus === 'siap produksi' ? 'status-badge status-green' :
                                        ($currentStatus === 'produksi siap diambil' ? 'status-badge status-blue' : 'status-badge status-bg-default');
                                } else {
                                    $badgeClass = $badgeMap[$currentStatus] ?? 'status-badge status-bg-default';
                                }
                            @endphp
